...
for row in orders:
    ...
    pred_labels, properties = model.process_image(
        input_image_file, output_path
    )

    pred_labels = (pred_labels == 2).astype(np.uint8)
    SimpleITKIO().write_seg(pred_labels, os.path.join(output_path, f"{row[0]}.nii.gz"), properties)
